# This code is originally from HCIPy: www.github.com/ehpor/hcipy
...
class GifWriter(object):
    ...
    def __del__(self):
        try:
            ...
            warnings.warn(
                "Something went wrong while closing the GifWriter...", RuntimeWarning
            )
...
class FFMpegWriter(object):
    ...
    def __init__(self, filename, codec=None, framerate=24, quality=None, preset=None):
        if codec is None:
            ...
            if extension == ".mp4" or extension == ".avi":
                ...
                raise ValueError(
                    "No codec was given and it could not be guessed based on the file extension."
                )
        ...
        if shutil.which(ffmpeg_path) is None:
            raise RuntimeError(
                "ffmpeg was not found. Did you install it and is it accessible, either from PATH or from the HCIPy configuration file?"
            )
    ...
    def __del__(self):
        try:
            ...
            warnings.warn(
                "Something went wrong while closing FFMpeg...", RuntimeWarning
            )
    ...
    def _repr_html_(self):
        ...
        if not self.is_closed:
            raise RuntimeError(
                "Attempted to show the generated movie on an opened FFMpegWriter."
            )
